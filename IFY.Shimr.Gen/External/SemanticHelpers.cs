...
// Based on https://raw.githubusercontent.com/Grauenwolf/Tortuga-TestMonkey/main/Tortuga.TestMonkey/Tortuga.TestMonkey/SemanticHelper.cs
// 2022-11-11
[ExcludeFromCodeCoverage] // Not our code
internal static class SemanticHelper
{
    [return: NotNullIfNotNull("str")]
    public static string? TrimStart(this string? str, string value, StringComparison comparisonType = StringComparison.Ordinal)
    {
        return str?.StartsWith(value, comparisonType) == true
    ...
    public static string GetName(this INamedTypeSymbol symbol)
    {
        ...
        if (symbol.ContainingType != null)
        {
            prefix = GetName(symbol.ContainingType)
                + "."; // TODO: "+" class, etc.
        }
        ...
        return prefix + symbol.Name + suffix + NullableToken(symbol);
    }
        ...
        for (var i = 0; i < typeArguments.Count; i++)
        {
            switch (typeArguments[i])
            {
                case IArrayTypeSymbol ats:
                    output.Add(FullName(ats.BaseType!) + "[]");
                    break;
