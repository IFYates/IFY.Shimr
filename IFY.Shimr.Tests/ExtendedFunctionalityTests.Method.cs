...
public class ExtendedFunctionalityTests_Method
{
    ...
    public class ProxyImpl_CallBase
    {
        ...
        public static void MethodB(ITestShim_CallBase obj)
        {
            ...
            {
                throw new System.InvalidOperationException("recursion");
            }

            MethodBCalledObj = obj;
            obj.MethodB(); // Calls original member of override
        }
    }

    [TestMethod]
    [Timeout(100)] // Incase of recursion
    ...
    public void Failure_during_underlying_call_does_not_break_proxy()
    {
        ...
        // Act
        try { shim.Fail(); } catch (System.InvalidOperationException) { ++fails; }
        try { shim.Fail(); } catch (System.InvalidOperationException) { ++fails; }
    ...
    public interface ITestShim_ChangeShimReturnType : ITestShim
    {
        [ShimProxy(typeof(ProxyImpl_ChangeShimReturnType))]
        new bool MethodA();
    }
    [ExcludeFromCodeCoverage]
    public class ProxyImpl_ChangeShimReturnType
    {
        public static ITestShim_ChangeShimReturnType MethodACalledObj { get; set; } = null!;
        public static bool MethodA(ITestShim_ChangeShimReturnType inst)
        {
            MethodACalledObj = inst;
            return true;
        }
    ...
    public void Can_change_shim_return_type_in_hierarchy()
    {
        // Arrange
        var obj = new TestClass_OnlyMethodA();
        var shim = obj.Shim<ITestShim_ChangeShimReturnType>();

        // Act
        var result = shim.MethodA();

        // Assert
        Assert.IsFalse(obj.MethodACalled);
        Assert.IsTrue(result);
        Assert.AreSame(shim, ProxyImpl_ChangeShimReturnType.MethodACalledObj);
    }
#endif
